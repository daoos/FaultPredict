...
class AttributeReduction:
    ...
    def get_NAH(self, relative_attr_set, attr_set, sigma):
        ...
        """
        if not attr_set:
            return np.log2(len(self.universe))
        # 对象在条件属性子集B下的邻域
...
if __name__ == '__main__':
    ...
    NAH_List = dict()
    for a in C_array:
        NAH_List[a] = ar.get_NAH(D_array, [a], sigma)
    R = []
    # sort函数按照值进行s排序，通过下标-1选出值最大的键值对，通过下标0拿到键值对的键
    for key, value in sorted(NAH_List.items(), key=lambda item: item[1]):
        print(list(set(C_array) - {key}))
        if NAH_D_C < ar.get_NAH(D_array, list(set(C_array) - {key}), sigma):
            R.append(key)
    print(sorted(R))
